...
const CourseContentMedia = ({data, id, activeVideo, setActiveVideo, user, refetch}: Props) => {
    ...
    // Get Add Answer In Question Mutation
    ...
    return (
       ...
       <h1 className="pt-2 text-[25px] font-[600] dark:text-white text-black ">
        ...
        {["Overview", "Resources", "Q&A", "Reviews"].map((text, index) => (
          <h5 key={index} className={`800px:text-[20px] cursor-pointer ${activeBar === index ? "text-[#00df9a]" : "dark:text-white text-black"}`}
                  ...
                  return (
                    <div className="w-full my-5 dark:text-white text-black" key={index}>
                      <div className="w-full flex">
                        <div>
                          <Image src={item.user.avatar ? item.user.avatar.url : "/profile.jpg"}
                      ...
                      {item.commentReplies.map((i: any, index: number) => (
                        <div className="w-full flex 800px:ml-16 my-5" key={index}>
                          <div className="w-[50px] h-[50px]">
                            <Image src={i.user.avatar ? i.user.avatar.url : "/profile.jpg"}
                              width={50} height={50} alt="" className="w-[50px] h-[50px] rounded-full object-cover"/>
                          </div>
                          <div className="pl-2">
